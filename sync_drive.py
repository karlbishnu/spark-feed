...
# 폴더 ID 정제: 앞뒤 공백/따옴표 제거 및 전체 URL 입력 시 ID만 자동 추출
raw_folder_id = os.environ["DRIVE_FOLDER_ID"].strip().strip('"').strip("'")
if "drive.google.com" in raw_folder_id:
    match = re.search(r'folders/([a-zA-Z0-9_-]+)', raw_folder_id)
    FOLDER_ID = match.group(1) if match else raw_folder_id
else:
...
# 지정된 폴더 내 파일 목록 조회 (공유 드라이브 지원 옵션 포함)
...
for file in files:
    ...
    # 설정 파일(_topics.md 등)은 피드 동기화에서 제외
